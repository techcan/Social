...
public class LocalityList extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		dialog = new ProgressDialog(LocalityList.this);
		mListView = (ListView) findViewById(R.id.listview_main12);
		Intent intent = getIntent();
		id = intent.getStringExtra("cityid");
		dialog.setMessage("Loading...\nPlease Wait....");
		mCustomAdapterList = new CustomAdapterList(LocalityList.this,
				_mArrayList);
		...
		mListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				Intent intent = new Intent(LocalityList.this,
						SubLocality.class);
			...
			//	intent.putExtra("longitude", checkList.get(position)
					//	.getLongitude());
				intent.putExtra("zone_id", checkList.get(position)
						.getZone_id());
	...
	private void CallingVolley_city() {
		...
		String url = "https://api.zomato.com/v1/zones.json?city_id="+id;
	...
	protected void parsingMethod(String value) {
		...
		try {
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				restaurantCityModel.setCity_id(jsonObject3.getString("city_id"));
